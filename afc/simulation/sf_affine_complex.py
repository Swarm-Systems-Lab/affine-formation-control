...
class sim_frame_affine_complex:
    def __init__(
        ...
        self.data = {"t": None, "p": None, "p_perp_norm": None}
        self.debug = debug
        ...
        # S projector
        A = np.array(
            [np.ones(self.n), np.real(self.p_star_c), np.imag(self.p_star_c)]
        ).T
        self.Ps = A @ np.linalg.inv(A.T @ A) @ A.T
        ...
        self.B = gen_inc_matrix(self.n, Z)
    ...
    def numerical_simulation(self):
        ...
        for i in tqdm(range(its)):
            tdata[i] = self.simulator.t
            pdata_c[i, :] = self.simulator.p
            pperpnormdata[i] = np.linalg.norm(
                (np.eye(self.n) - self.Ps) @ self.simulator.p
            )
        ...
        self.data["t"] = tdata
        self.data["p"] = pdata
        self.data["p_perp_norm"] = pperpnormdata
    ...
    def plot(self, ax=None, lim=20):
        ...
        for i in range(self.n):
            ...
            ax.plot(pdata[-1, i, 0], pdata[-1, i, 1], ".", c=colors[i], label=(i + 1))
